...
const DealPage = ({ deal }: Props) => {
    return (
        <>
            <section className="container px-4 mx-auto pt-10">
                <h3 className="text-xl font-medium">{deal.offer.title}</h3>
            </section>
                    ...
                    <Card className="" title="Status">
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center gap-2">
                                <Icon icon="flag" className={`w-6 h-6 ${deal.deal.status === "started" ? "text-violet-800" : "text-slate-300"}`} />
                                <p className={`text-base font-medium ${deal.deal.status === "started" ? "text-slate-900" : "text-slate-400"}`}>Waiting for user payment</p>
                            </div>

                            <div className="flex items-center gap-2">
                                <Icon icon="clock" className={`w-6 h-6 ${deal.deal.status === "user_paid" ? "text-violet-800" : "text-slate-300"}`} />
                                <p className={`text-base font-medium ${deal.deal.status === "user_paid" ? "text-slate-900" : "text-slate-400"}`}>Waiting for delivery</p>
                            </div>
                    ...
                    <div className="p-4 flex flex-col gap-8 bg-slate-100 rounded-xl">
                        ...
                        <div className="grid grid-cols-[repeat(auto-fill,_minmax(250px,_1fr))] gap-4">
                            <div className="bg-white flex flex-col gap-4 px-2 py-3 rounded-2xl">
                                <div className="w-full aspect-[3/2] relative rounded-lg overflow-clip">
                                    <Image className="object-cover" fill src={deal.offer.imageUrl} alt="Dota 2 thumbnails" />
                                </div>
                                <h5 className="text-sm font-medium">{deal.offer.title}</h5>
                                <p className="text-slate-600 text-sm">{deal.offer.description}</p>
                                <div className="flex items-center gap-2 justify-end text-violet-600 ">
                                    <span className="text-base font-medium"><span className="text-xl">{deal.offer.price}</span></span>
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2 text-violet-600">
                                <span className="text-2xl font-medium">{deal.offer.price}</span>
                                <Icon icon="flash" className="w-4 h-4" />
                            </div>
                        </div>
                    </div>
