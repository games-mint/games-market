...
const DealsCard = ({ deals }: Props) => {
    ...
    const dealsToShow = useMemo(() => deals.filter(el => activeStatus === "compeleted" ? el.deal.status === "completed" : el.deal.status !== "completed"), [activeStatus, deals]);
        ...
        <Card
            ...
            header={
                <div className='flex items-center'>
                    ...
                    <button onClick={() => setActiveStatus('compeleted')} className={`px-4 py-2 ${activeStatus === 'compeleted' ? "text-violet-600 bg-violet-200" : "text-slate-500"} rounded-full text-sm whitespace-nowrap font-medium`}>Completed</button>
                </div>
            }
            title='Your deals'
        >
            <div className='grid grid-cols-[repeat(auto-fill,_minmax(170px,_1fr))] gap-4'>
                {dealsToShow.map(({ deal, seller, offer }) => (
